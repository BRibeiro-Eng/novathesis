...
\begin{figure}[tbp]
  ...
  futura; a banda $\pm 2s$ é a convenção da plataforma. Com $n=40$ as duas
  últimas quase coincidem, e é o intervalo da média --- aqui $6{,}4$ vezes
  mais estreito --- que é rotineiramente confundido com elas. \textbf{B}: a
  ...
  civil completo, 365. A convenção aproxima bem o nominal
  da equação~\ref{eq:mat-dois-sigma} quando o ajuste tem um ano de dados
  ($95{,}35\,\%$ no centro), e afasta-se nas células pequenas
  ($94{,}1\,\%$ com $n=30$, $92{,}9\,\%$ no extremo da gama de carga).}}
...
\begin{figure}[tbp]
  ...
  {\matref{A diferença entre exigir as duas somas em separado e exigir
  sucessos conjuntos. O exemplo é construído e a enumeração é exata; nenhum
  valor da refinaria entra nesta figura. \textbf{A}: os dois vetores
  binários do texto sobre os cinco pares anuais encadeados. As somas de
  $G_j$ e de $C_j$ são ambas quatro, pelo que a regra da
  equação~\ref{eq:mat-gate-historico} se cumpre; a terceira linha mostra
  que só três pares satisfazem os dois critérios ao mesmo tempo.
  \textbf{B}: as seis configurações admissíveis de cada vetor --- as que
  somam quatro ou cinco --- cruzadas duas a duas. A azul, as combinações
  que também teriam quatro sucessos conjuntos; a cinzento, as que não
  teriam. A moldura assinala o exemplo do painel A. Das 36 configurações
  que passam a regra histórica, 20 --- $56\,\%$ --- falhariam uma regra de
  sucesso conjunto com o mesmo limiar nominal, o que mede quanto as duas
  definições diferem e justifica identificar qual foi usada.}}
