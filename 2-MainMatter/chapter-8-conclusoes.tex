...
detetadas. \revisto{As causas distribuíam-se pelo motor de ingestão, pelo mapeamento e pelo próprio relatório oficial.} O fuel gás
...
essa deriva. \revisto{Nos três níveis de vapor, não se demonstrou um ganho preditivo consistente da reta anual sobre a média do treino:} o \emph{skill} do vapor de 24 e de
10~bar fica próximo de zero, e o do vapor de 3~bar muda de sinal. \revisto{No vapor de 24~bar, não se rejeita a igualdade dos declives e a cobertura é compatível com a nominal; estes resultados não demonstram estabilidade nem ausência de relação com a carga;} o de 10~bar tem um nível que sobe de ano para ano; o
...
validade nunca declarado; \revisto{os resíduos têm autocorrelação de primeira ordem entre 0,46 e 0,90, o que compromete a inferência sob independência e aumenta a incerteza da cobertura estimada da banda;} e a reestimação anual acomoda nos parâmetros a deriva que o
...
\revisto{com três em cinco, quatro dos 16 candidatos de mínimos quadrados passariam o critério de conteúdo preditivo, e nenhum seria aceite, por nenhum deles ter banda suportada.}
...
\paragraph{A revisão.} \revisto{Entre as 162 publicações com classificação concordante das famílias de modelos, de um total de 331 elegíveis, o rácio de intensidade é a família mais frequente,} embora a regressão apareça
...
validação fora da amostra aparece em quatro. \revisto{Na extração dos mecanismos, 89 das 130 ocorrências não nomeiam uma causa física ou operacional}
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{3.3cm}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X@{}}
    \toprule
    \textbf{Condição}  &  \textbf{Antes da plataforma}  &  \textbf{No fim do trabalho} \\
    \midrule
...
que a da referência, e \revisto{o critério de aceitação recusou as combinações avaliadas; a aceitação de modelos mais complexos continua a exigir uma avaliação própria segundo critérios previamente declarados.} O roteiro organiza-se por isso a partir das falhas observadas, e começa
...
\hiderowcolors
\toprule
\textbf{Falha observada}  &  \textbf{Opção}  &  \textbf{Dados adicionais}  &  \textbf{Comparador}  &  \textbf{Critério de sucesso} \\
\midrule
\endfirsthead
\toprule
\textbf{Falha observada}  &  \textbf{Opção}  &  \textbf{Dados adicionais}  &  \textbf{Comparador}  &  \textbf{Critério de sucesso} \\
