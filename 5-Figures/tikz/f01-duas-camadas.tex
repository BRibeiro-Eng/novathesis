\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[x=1cm, y=1cm, font=\scriptsize]
    ...
    \node[ntcaixa, text width=2.5cm, minimum height=8mm] (s3) at (1.4,-0.35)
      {Ficheiros e\\relatórios mensais};
    ...
    \node[ntcaixa, text width=2.2cm, minimum height=11mm] (dic) at (12.7,0.70)
      {Dicionário,\\origem, \emph{flags}\\e versões};
    ...
    % ----------------------------------------- faixa 2: analitica -----
    \node[ntdestaque, text width=2.6cm, minimum height=12mm] (ref) at (1.65,-2.90)
    ...
    % --------------------------------------- proposta por testar ------
    \node[ntproposta, text width=12.6cm, minimum height=7mm] (aval) at (6.85,-4.55)
      {\textbf{Proposta ainda por testar} (Cap.~\ref{cha:discussao}):
       separar referência fixa, estado estimado e monitor declarado};
    \draw[ntsetat] (aval.north) -- ++(0,0.32);

    \draw[-{Stealth[length=4pt,width=3pt]}, draw=corTinta, line width=.6pt]
      (-0.05,-5.25) -- ++(0.6,0);
    \node[ntmini, anchor=west] at (0.6,-5.25) {em produção na refinaria};
    \draw[ntsetat] (4.3,-5.25) -- ++(0.6,0);
    \node[ntmini, anchor=west] at (4.95,-5.25) {proposta ainda por testar};
  \end{tikzpicture}
  ...
  {As duas camadas da fiabilidade do desempenho energético e os capítulos
   ...
   e um desvio, por si, não identifica a sua causa. A traço contínuo, o que
   está em produção; a tracejado, a arquitetura proposta no
   Capítulo~\ref{cha:discussao}, que este trabalho não chegou a implementar.}
