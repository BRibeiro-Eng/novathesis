...
\alterado{\paragraph{Nomenclatura e citações.} A ISO~50001:2018 foi adotada
...
Lidas em conjunto, a 50001 (requisitos), a 50006 (construção de EnPI/EnB) e a 50015 (M\&V) desenham uma cadeia de medição: uma sequência de operações que transforma dados de processo em declarações sobre desempenho energético e, no limite, em alarmes e decisões. A cadeia nunca é apresentada como tal em nenhuma das normas --- cada documento descreve o seu troço --- mas a sua reconstrução explícita (Figura~\ref{fig:cadeia-normativa}) é o instrumento analítico deste capítulo, porque permite perguntar, elo a elo, três coisas: o que é exigido, o que é deixado à organização, e que pressuposto tácito o elo precisa de importar para funcionar. \alterado{A figura fixa a cadeia e os pressupostos; a Tabela~\ref{tab:matriz-rastreabilidade} responde, cláusula a cláusula, ao que é exigido e ao que fica por especificar.}
...
\alterado{Esta atribuição precisa é essencial, e delimita o que se critica.
...
\alterado{Chamamos assimetria normativa ao desequilíbrio entre a densidade
prescritiva das normas quanto à \emph{existência} dos instrumentos de
medição do desempenho energético e o seu silêncio quanto à \emph{validade}
desses instrumentos. A Secção~\ref{sec:enb-cadeia} percorreu-o elo a elo e a
...
verificação dos pressupostos dos modelos ou fornece critérios operacionais
para detetar que uma referência deixou de ser válida. Daqui vem a
consequência que o resto do capítulo persegue: a ausência de uma barreira
normativa não é evidência de validade estatística, e as normas analisadas
não contêm o mecanismo que permitiria distinguir uma coisa da outra. O
...
Secção~\ref{sec:processos-continuos}. \alterado{Diga-se com clareza: os
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}c >{\raggedright\arraybackslash}p{2.1cm} X X X c@{}}
    ...
    \addlinespace
    2 & \alterado{50001 \S6.3~c)\,1), \S3.4.9~N1}; 50006 \S4.2.4 & Determinar variáveis relevantes (métodos gráficos) & Limiar de significância; teste de \emph{suficiência} do conjunto & As variáveis descrevem o estado operacional & E6 \\
    \addlinespace
