...
\label{sec:mat-redes}
% MAP: M03.05 M03.06
A matriz $B$ de incidência artigo--termo origina a projeção $W=B^{\mathsf T}B$. Fora da diagonal, $W_{jk}$ é o número de artigos que contêm ambos os termos; a diagonal regista a ocorrência de cada termo e é retirada quando se desenham apenas ligações entre termos distintos. Limiares de ocorrência e limites ao número de nós alteram a rede representada e devem ser declarados.

Se $A_{ij}=1$ quando o artigo $i$ cita o artigo $j$, $A$ descreve citação direta. Para uma matriz $C$ artigo--referência, $CC^{\mathsf T}$ conta referências partilhadas entre artigos e $C^{\mathsf T}C$ conta cocitações entre referências. O acoplamento normalizado usado na análise exploratória é
\begin{equation}
...
\quad \boldsymbol\beta=(a,b)^{\mathsf T}.
...
=(X^{\mathsf T}X)^{-1}X^{\mathsf T}\boldsymbol E.
\label{eq:mat-ols}
\end{equation}
...
O modelo alargado escreve-se $E_t=a+bQ_t+\boldsymbol\gamma^{\mathsf T}\boldsymbol Z_t+\varepsilon_t$. Comparar modelos encaixados permite medir o contributo incremental das covariáveis, desde que se usem os mesmos dias. O $RSS$ de treino não aumenta ao acrescentar colunas a um modelo de mínimos quadrados; essa propriedade não assegura ganho em aplicação. As referências B01, B02, B03, B08 e B09 correspondem, respetivamente, à constante, à reta com carga e intercepto, aos conjuntos declarados de covariáveis nucleares e de referência e à reta pela origem. Cada conjunto deve ser enumerado no dicionário do lote, incluindo transformações e disponibilidade temporal.
...
+\sum_j\lambda_j\boldsymbol\theta^{\mathsf T}S_j\boldsymbol\theta.
...
\begin{equation}
\widehat V_{HAC}=\frac{n}{n-p}(X^{\mathsf T}X)^{-1}
\left[\sum_{t,s}K\!\left(\frac{|t-s|}{b_H}\right)
\boldsymbol g_t\boldsymbol g_s^{\mathsf T}\right](X^{\mathsf T}X)^{-1}.
\label{eq:mat-hac}
\end{equation}
...
\begin{equation}
W=(C\widehat{\boldsymbol\beta}-\boldsymbol c)^{\mathsf T}
(C\widehat V C^{\mathsf T})^{-1}
...
\begin{tcolorbox}[breakable,colback=black!2,colframe=black!45,title={Algoritmo G.1 --- Intervalo para um contraste emparelhado}]
...
% MAP: M08.01 M08.09
Uma diferença padronizada entre grupos $A$ e $B$ pode escrever-se
\begin{equation}
SMD=\frac{\bar x_A-\bar x_B}{\sqrt{(s_A^2+s_B^2)/2}}.
\label{eq:mat-smd}
\end{equation}
O sinal indica a direção da diferença. O denominador deve ser explicitado: há convenções ponderadas pelas dimensões dos grupos que produzem outro valor. Dispersão nula ou muito pequena impede uma leitura estável. A separação de dias retidos e removidos por este critério descreve a seleção; não identifica, por si só, um regime físico.
...
\label{sec:mat-acf}
% MAP: M08.02 M08.03
Para uma série estacionária, $\rho_k=\operatorname{Cov}(e_t,e_{t-k})/\operatorname{Var}(e_t)$. Uma estimativa no calendário usa apenas pares $\mathcal P_k=\{t:t,t-k\in\mathcal S\}$. Deve declarar-se se a normalização usa a variância global ou as variâncias dos pares. A ACF mostra associações por desfasamento; a PACF remove os contributos lineares dos desfasamentos intermédios, podendo ser obtida pela recursão de Durbin--Levinson quando a sequência de autocovariâncias é admissível. Lacunas e estimativas por pares podem comprometer essa admissibilidade.
...
\label{eq:mat-dw-lb}
\end{equation}
Estas fórmulas supõem uma sequência regularmente indexada. A substituição de pares consecutivos por pares civis disponíveis altera os denominadores pertinentes e a distribuição de referência. A correção dos graus de liberdade depende ainda do modelo ajustado. Os valores $p$ clássicos não devem ser interpretados como calibrados sem verificar estas condições\parencite{BoxJenkins2015}.
...
% MAP: M08.10 M08.11
A alavancagem é a diagonal $h_{ii}$ da matriz de projeção $H=X(X^{\mathsf T}X)^{-1}X^{\mathsf T}$. A distância de Cook combina alavancagem e resíduo:
\begin{equation}
...
\revisaotese{MAT-06}{P1}{METODO}{Uniformizar as convenções dos diagnósticos}{Confirmar no produtor das tabelas a fórmula de SMD, a normalização da ACF civil, os pares usados em Durbin--Watson e Ljung--Box e os graus de liberdade dos testes auxiliares. As expressões clássicas acima servem de referência; qualquer adaptação à série irregular deve ser transcrita antes de atribuir calibração aos valores $p$. Acrescentar as fontes primárias específicas dos diagnósticos mantidos na versão final.}
...
\sqrt{\boldsymbol x_0^{\mathsf T}(X^{\mathsf T}X)^{-1}\boldsymbol x_0},
\label{eq:mat-ic-media}\\
\widehat E_0&\ \pm\ t_{n-p,1-\alpha/2}\,s
\sqrt{1+\boldsymbol x_0^{\mathsf T}(X^{\mathsf T}X)^{-1}\boldsymbol x_0}.
...
=\arg\min_{\boldsymbol\beta}\sum_t\rho_\tau(E_t-\boldsymbol x_t^{\mathsf T}\boldsymbol\beta).
\label{eq:mat-quantil}
\end{equation}
Dois ajustes produzem $L_t=\boldsymbol x_t^{\mathsf T}\widehat{\boldsymbol\beta}_{\tau_L}$ e $U_t=\boldsymbol x_t^{\mathsf T}\widehat{\boldsymbol\beta}_{\tau_U}$. A condição $L_t\leq U_t$ deve ser verificada no domínio de aplicação. Recusar um candidato com cruzamentos é uma regra de admissibilidade; impor o não cruzamento durante a estimação é outro procedimento\parencite{Chernozhukov2010}. A não convergência deve ser registada como tal. Quantis muito extremos estimados em janelas anuais podem apoiar-se em poucos dias de cauda.
...
Para tendência local, introduz-se $b_t^{\mathrm{estado}}$: $\ell_t=\ell_{t-1}+b_{t-1}^{\mathrm{estado}}+\eta_t$ e $b_t^{\mathrm{estado}}=b_{t-1}^{\mathrm{estado}}+\zeta_t$. A transição matricial de $(\ell_t,b_t^{\mathrm{estado}})^{\mathsf T}$ usa $T=\left(\begin{smallmatrix}1&1\\0&1\end{smallmatrix}\right)$ e a previsão da covariância usa $TP T^{\mathsf T}+V$. O estado inicial e as variâncias são estimados na referência e mantidos fixos na aplicação descrita. O estado filtrado usa dados até $t$; o estado alisado usa também o futuro e só pode ser apresentado como reconstrução retrospetiva.
...
\draw[arr] (res.south east)--(state.north west);
\end{tikzpicture}
...
\begin{tcolorbox}[breakable,colback=black!2,colframe=black!45,title={Algoritmo G.2 --- Atualização de um monitor}]
\begin{enumerate}[leftmargin=*,nosep]
...
O diagnóstico por estratos usa cortes de carga fixados no treino e calcula o desvio médio em cada estrato. Se a média for padronizada, devem publicar-se a escala de referência do estrato e a regra de dimensão efetiva usada. As versões por estratos e por declive não são intercambiáveis: a disponibilidade de cada estrato muda com a operação.
...
\begin{tcolorbox}[breakable,colback=black!2,colframe=black!45,title={Algoritmo G.3 --- Avaliação com verdade conhecida}]
\begin{enumerate}[leftmargin=*,nosep]
...
\end{equation}
A matriz usada deve ser semidefinida positiva; qualquer regularização deve ficar registada. Com uma matriz de transição geral $A$, a covariância estacionária obedece a $\Sigma=A\Sigma A^{\mathsf T}+V_\eta$ e exige verificar estabilidade e admissibilidade de $V_\eta$.
