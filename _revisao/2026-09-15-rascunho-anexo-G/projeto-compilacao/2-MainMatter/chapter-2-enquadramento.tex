...
%% Seccoes 2.1-2.3 preenchidas em 2026-08-28 (rascunho v0 -> LaTeX)
...
% [REV-2026-09-13][C2-04][P1][NORMAS]
\revisaotese{C2-04}{P1}{NORMAS}
{Adequação normativa não se reduz à existência do instrumento}
{O próprio texto cita requisitos de adequação dos EnPI e demonstração de
melhoria. Da ausência de limiares universais não decorre que um método
inválido seja conforme ou que uma auditoria apenas verifique existência.
Distinguir: ausência de teste quantitativo prescrito; responsabilidade
da organização pela adequação; conclusão de auditoria, aqui não avaliada.
Reformular "o método é conforme" para "a forma do modelo e a revisão
periódica não são, por si, excluídas pelas cláusulas analisadas".
Aplicar a mesma restrição à Introdução e à Sec. 7.2.}%
% [/REV-2026-09-13]
Esta atribuição precisa é essencial: o objeto da crítica do Capítulo~\ref{cha:diagnostico} não é a conformidade --- o método em produção \emph{é} conforme, localiza-se dentro do espaço que as normas permitem. O objeto da crítica é o facto de o espaço permitido conter, sem qualquer barreira normativa, implementações cujos pressupostos estatísticos são sistematicamente violados pelos dados a que se aplicam --- e de as normas não fornecerem ao operador nenhum instrumento que o alerte para isso. Dito de outro modo: a conformidade normativa não é evidência de validade estatística, e as normas não contêm o mecanismo que permitiria distinguir uma da outra.

\begin{figure}[htbp]
  \centering
  \missingfigure[figwidth=0.85\textwidth]{FIG 2.3 --- Redesenho anotado do conceito EnPI/EnB/meta (a partir da Figura 1 da ISO 50006): eixo temporal com período de baseline e de reporte, valor de referência e valor corrente do EnPI, melhoria e meta. Anotação 1: as bandas de alarme em torno da baseline não aparecem em nenhuma figura normativa --- são extensão de implementação. Anotação 2: tudo pressupõe que a relação estimada à esquerda continua válida à direita.}
  \caption[O conceito EnPI/EnB/meta e a extensão de implementação]{O conceito EnPI/EnB/meta e a fronteira entre o que a norma desenha e o que a implementação acrescenta. Adaptado de \cite[Figura~1]{ISO50006_2014}.}
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}c >{\raggedright\arraybackslash}p{2.1cm} X X X c@{}}
    ...
    4 & 50006 \S4.4.3 & \enquote{Testar a validade} do EnB ($p$, $F$, $R^2$ nomeados) & \enquote{Se apropriado}; sem limiares; sem verificação de pressupostos dos testes & Resíduos independentes, homoscedásticos, normais & E1, E2, D \\
...
A leitura vertical da coluna dos pressupostos produz a lista compacta que estrutura o resto da dissertação: \textbf{suficiência} (as variáveis descrevem o estado), \textbf{forma} (a classe de modelo captura a relação), \textbf{representatividade} (o período de treino antecipa o futuro), \textbf{transportabilidade} (a relação não muda entre treino e aplicação), \textbf{calibração} (os desvios têm escala conhecida) e \textbf{detetabilidade} (a invalidação é observável). As cinco patologias do Capítulo~\ref{cha:diagnostico} são, uma a uma, violações empíricas destes pressupostos.

\subsection{O silêncio como escolha: comparação com o IPMVP e a ASHRAE Guideline 14}
\label{sec:assimetria-comparacao}
% [REV-2026-09-13][C2-06][P2][FONTES]
\revisaotese{C2-06}{P2}{FONTES}
{Comparar vias e edições sem extrapolar os limiares}
{Manter a distinção já feita entre simulação calibrada, regressão de
baseline e incerteza de poupanças; explicitar a via também em cada célula
da tabela. A recomendação R\textsuperscript{2} \textgreater{}= 0,75 do IPMVP aparece apoiada numa fonte
secundária e ainda por confirmar: não a apresentar como requisito ou
recomendação atual verificada. Identificar edição/cláusula consultadas.
"Critérios mais exigentes" exige comparação no mesmo estimando; os testes
da tese e os limiares de M\&V medem propriedades diferentes.}%
% [/REV-2026-09-13]
...
Primeira frente: \emph{limiares de calibração de modelos}. A ASHRAE Guideline~14 exige, como requisito de conformidade da via de simulação calibrada, um erro médio normalizado (NMBE) máximo de $\pm 5\,\%$ e um coeficiente de variação do erro quadrático médio, CV(RMSE), máximo de $15\,\%$ face a dados de calibração mensais --- $\pm 10\,\%$ e $30\,\%$, respetivamente, para dados horários~\cite[\S4.3.2.4\,f), \S5.3.3.3.10]{ASHRAE14_2014}. As orientações de M\&V do programa federal norte-americano (FEMP) adotam exatamente os mesmos limiares, por remissão declarada para a ASHRAE~\cite[\S4.5.3.2, Tabela~4-2]{FEMP_2015}. O IPMVP, que desde as edições de 2009 remete os detalhes técnicos de calibração para a Guideline~14~\cite{RuizBandera2017}, recomenda adicionalmente --- tal como o \emph{ASHRAE Handbook} --- que o $R^2$ de um modelo calibrado nunca seja inferior a $0{,}75$~\cite{RuizBandera2017}.\todo{Confirmar formulação na edição corrente do IPMVP (EVO 10000-1) quando o download for aprovado.}
...
Independentemente do juízo sobre estes limiares em concreto --- a literatura discute a sua suficiência estatística, e o Capítulo~\ref{cha:metodos} adotará critérios mais exigentes --- a sua simples existência demonstra que protocolos com ambição de aplicação geral \emph{conseguem} fixar critérios quantitativos de aceitação de modelos, regras de tratamento de dados e tetos de incerteza. A família ISO~50001 escolheu não o fazer. A Tabela~\ref{tab:comparacao-frameworks} sintetiza a comparação.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{3.2cm} X X X X@{}}
    ...
    Limiares quantitativos & Nenhum & Nenhum (testes/diagnósticos apenas nomeados) & $R^2 \geq 0{,}75$ (rec.); detalhe remetido para a G14 & NMBE $\pm5/\pm10\,\%$, CV(RMSE) $15/30\,\%$ (calibração, \S4.3.2.4); CV(RMSE) $\leq 20$--$30\,\%$ no modelo de regressão de base (\S4.3.2.1) \\
...
% [REV-2026-09-13][F02][P2][FIGURA]
\revisaotese{F02}{P2}{FIGURA}
{Quadro físico de mecanismos e variáveis observáveis}
{Depois do primeiro parágrafo desta secção, inserir uma tabela de quatro
colunas: mecanismo; efeito esperado em energia/carga; variável observável;
explicações alternativas. Incluir fouling do pré-aquecimento, composição
do crude, severidade de separação, integração térmica e balanço entre
níveis de vapor. Distinguir mecanismos gerais dos efetivamente presentes
na CDU. Esta tabela liga a física às escolhas de covariáveis e evita
que sinais de regressão sejam interpretados como leis causais.}%
% [/REV-2026-09-13]

%% >> Fouling, desativacao de catalisador e variabilidade da carga como nao-estacionariedade estrutural que nao e ineficiencia.
%% >> Fecho: cada mecanismo fisico viola um dos seis pressupostos da Tabela~\ref{tab:matriz-rastreabilidade}.


\section{O caso de estudo: a refinaria de Sines e a unidade de destilação atmosférica}
\label{sec:caso-estudo}
% [REV-2026-09-13][F03][P2][FIGURA]
\revisaotese{F03}{P2}{FIGURA}
{Diagrama de processo e fronteira energética da CDU}
{Figura essencial, logo após apresentar a unidade: crude -\textgreater{} pré-aquecimento
-\textgreater{} dessalgação -\textgreater{} forno -\textgreater{} coluna -\textgreater{} produtos, ajustado ao esquema real.
Assinalar entradas/saídas de fuel gás, vapor 24/10/3 bar e eletricidade,
retornos e produção creditada, fronteira do indicador e pontos de medição.
Sem caudais inventados; se não houver dados autorizados, esquema funcional
sem escala. Legenda distingue consumo bruto, líquido e alocado. Desenho
vetorial, uma página no máximo. Mais útil aqui que uma fotografia geral
da refinaria; uma fotografia só se identificar equipamento relevante.}%
% [/REV-2026-09-13]

% [REV-2026-09-13][C2-07][P2][CONTEUDO]
\revisaotese{C2-07}{P2}{CONTEUDO}
{Contextualizar seleção da unidade e energia equivalente}
{Acrescentar função/capacidade apenas com fonte, razão documentada da escolha,
perímetro material e energético, mistura/reprocessamento de carga,
principais consumidores e utilidades partilhadas. Explicar GNE como
convenção de contabilização e EII como referência de benchmarking.
Não confundir equivalência energética, energia primária e exergia.
Citar literatura de refinação/integração térmica para os mecanismos;
a bibliografia atual está concentrada em normas e estatística.}%
% [/REV-2026-09-13]
%% >> A CDU, os cinco vectores energeticos, o EII e o metodo de baseline em producao.
%% >> Devolver a bola a sintese 2.3.4: "este e o objeto que o resto da dissertacao disseca".
