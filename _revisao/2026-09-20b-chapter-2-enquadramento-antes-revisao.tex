...
%% 2026-09-19: varredura previa a revisao integral -- \todo retirados (ver
%% bloco de pendencias no fim), Tabela 2.1 e Figura 2.2 novas, caixa C2-05
%% resolvida, nomenclatura fixada em EnPI/EnB (paragrafo "Nomenclatura e
%% citacoes" na 2.1). Versoes anteriores em _revisao/2026-09-19*-cap2-antes.tex.
%% ====================================================================
...
A gestão de energia industrial encontra-se hoje codificada num corpo normativo estruturado em torno da ISO~50001, que especifica os requisitos de um sistema de gestão de energia (SGE), e de um conjunto de normas de suporte, desenvolvidas pelo comité técnico ISO/TC~301, que operacionalizam componentes específicos desse sistema. Para o presente trabalho relevam três documentos: a ISO~50001:2018~\cite{ISO50001_2018}, que estabelece os requisitos do sistema; a ISO~50006~\cite{ISO50006_2014}, que fornece princípios e orientações para a medição do desempenho energético através de indicadores de desempenho energético (EnPI) e consumos energéticos de referência (EnB); e a ISO~50015~\cite{ISO50015_2014}, que estabelece princípios gerais para a medição e verificação (M\&V) do desempenho energético de organizações. A distinção de estatuto entre estes documentos é essencial para a análise que se segue: a ISO~50001 é uma norma de requisitos, certificável, redigida na forma verbal \enquote{deve}; a ISO~50006 e a ISO~50015 são normas de orientação, redigidas na forma \enquote{deverá/poderá}, sem carácter vinculativo --- a própria ISO~50015 declara que os seus princípios não são exigidos pela ISO~50001~\cite[Introdução]{ISO50015_2014}. A ISO~50006 e a ISO~50015 são aqui lidas nas edições de 2014, as consultadas. A ISO~50006 tem desde 2023 uma segunda edição, que substituiu a de 2014 e não foi consultada; o que este capítulo diz sobre o texto da ISO~50006 vale para a edição de 2014 e não se estende, sem verificação, ao quadro atual (nota no fim da Secção~\ref{sec:assimetria-comparacao}).

\paragraph{Nomenclatura e citações.} A ISO~50001:2018 foi adotada
em Portugal um ano depois, como NP~EN~ISO~50001:2019, que traduz
\emph{energy performance indicator} por indicador de desempenho energético
(IDE) e \emph{energy baseline} por consumo energético de referência (CER).
Esta dissertação usa em todo o texto as siglas inglesas, EnPI e EnB: são as
do original das três normas, as da literatura analisada no
Capítulo~\ref{cha:revisao} --- cujo corpus é integralmente em inglês --- e
as que a documentação técnica da plataforma usa. As passagens da ISO~50001
citam-se pela versão portuguesa, que foi a consultada; as da ISO~50006 e da
ISO~50015 são tradução do autor a partir do original inglês. Transcreve-se
onde a formulação literal é objeto de análise.

A ISO~50001 estrutura o SGE sobre o ciclo de melhoria contínua \emph{Plan-Do-Check-Act} (PDCA). Na fase de planeamento, a organização realiza uma avaliação energética (\S6.3), identifica os usos significativos de energia (USE), determina EnPI \enquote{adequados para medir e monitorizar o seu desempenho energético} e que \enquote{permitam demonstrar a melhoria do desempenho energético} (\S6.4), e estabelece EnB a partir da avaliação energética, tendo em conta um período de tempo adequado (\S6.5). Na fase de verificação, a organização monitoriza, mede, analisa e avalia o desempenho (\S9.1): a melhoria do desempenho energético deve ser avaliada comparando
os valores dos EnPI com os EnB correspondentes, e a organização
\enquote{deve investigar e responder a desvios significativos no desempenho
energético} (\S9.1.1). O par EnPI--EnB é, portanto, o mecanismo central através do qual a norma torna o desempenho energético mensurável e a melhoria demonstrável --- e é sobre este par que incide toda a análise desta dissertação.
A Tabela~\ref{tab:pdca-50001} localiza as cláusulas no ciclo e
...
{\footnotesize
\ntzebra
\begin{xltabular}{\textwidth}{@{}>{\raggedright\arraybackslash}p{1.15cm}>{\raggedright\arraybackslash}p{3.5cm}X>{\raggedright\arraybackslash}p{2.35cm}@{}}
\caption[A ISO 50001:2018 pelo ciclo PDCA]{Estrutura da ISO~50001:2018
  ...
  EnPI--EnB e que esta dissertação examina; entre parênteses, a norma de
...
\hiderowcolors
\toprule
\textbf{Fase} & \textbf{Cláusula} & \textbf{O que a norma exige} & \textbf{Nesta dissertação} \\
\midrule
\endfirsthead
\toprule
\textbf{Fase} & \textbf{Cláusula} & \textbf{O que a norma exige} & \textbf{Nesta dissertação} \\
...
\showrowcolors
  \emph{Plan} & 4.1--4.4 Contexto da organização & Determinar o âmbito e as fronteiras do sistema de gestão de energia & --- \\
   ...
   & \textbf{6.4 Indicadores de desempenho energético} (ISO~50006) & Determinar EnPI adequados para medir e monitorizar o desempenho e que permitam demonstrar a melhoria & Secção~\ref{sec:enb-cadeia}, elo 4 \\
   & \textbf{6.5 Consumo energético de referência} (ISO~50006) & Estabelecer o EnB; normalizar quando as variáveis relevantes afetam significativamente o desempenho; rever o EnB em três casos & Secção~\ref{sec:enb-cadeia}, elos 5 e 7 \\
  ...
  \midrule
  \emph{Check} & \textbf{9.1 Monitorização, medição, análise e avaliação} (ISO~50015) & Comparar os valores dos EnPI com os EnB correspondentes; investigar e responder a desvios significativos & Secção~\ref{sec:enb-cadeia}, elo 7; Capítulo~\ref{cha:diagnostico} \\
...
O conceito central é simples: um EnPI é uma \enquote{medida ou unidade do desempenho energético, conforme definido pela organização} (ISO~50001, \S3.4.4); o EnB é \enquote{referência(s) quantitativa(s) que fornecem uma referência para comparação do desempenho energético} (\S3.4.7), construída com dados \enquote{obtidos de um período específico de tempo e/ou condições, conforme definido pela organização} (\S3.4.7, Nota~1). A melhoria do desempenho energético demonstra-se comparando o valor do EnPI no período de reporte com o valor de referência do período de \emph{baseline}~\cite[Figuras~1 e~3]{ISO50006_2014}. Quando variáveis relevantes afetam significativamente o desempenho, a comparação direta deixa de ser válida e a organização \enquote{deve realizar a normalização} dos valores do EnPI e do EnB correspondente (ISO~50001, \S6.5) --- tipicamente através de um modelo que descreve a relação entre consumo e variáveis relevantes no período de \emph{baseline}~\cite[\S4.5.1 e Anexo~D]{ISO50006_2014}.

Repare-se, desde já, em duas expressões que a redação normativa usa com naturalidade e que carregam todo o peso estatístico do edifício: a comparação deve fazer-se \emph{em condições equivalentes} (ISO~50006, \S3.13), e o período de \emph{baseline} deve \emph{capturar a variabilidade dos padrões operacionais} (\S4.4.2). Ambas pressupõem que as condições operacionais são descritíveis pelas variáveis escolhidas e que a variabilidade observada num período --- tipicamente doze meses (\S4.4.2) --- é representativa da variabilidade futura. São pressupostos de estacionariedade: não da série de consumo, mas da
\emph{relação} entre consumo e variáveis relevantes \alterado{--- e de
estacionariedade em tudo menos naquilo que se procura, porque é a alteração
do desempenho que a comparação existe para revelar}. Não são enunciados como pressupostos em parte alguma do texto normativo.

\subsection{A cadeia de medição implícita}
\label{sec:enb-cadeia}

Lidas em conjunto, a 50001 (requisitos), a 50006 (construção de EnPI/EnB) e a 50015 (M\&V) desenham uma cadeia de medição: uma sequência de operações que transforma dados de processo em declarações sobre desempenho energético e, no limite, em alarmes e decisões. A cadeia nunca é apresentada como tal em nenhuma das normas --- cada documento descreve o seu troço --- mas a sua reconstrução explícita (Figura~\ref{fig:cadeia-normativa}) é o instrumento analítico deste capítulo, porque permite perguntar, elo a elo, três coisas: o que é exigido, o que é deixado à organização, e que pressuposto tácito o elo precisa de importar para funcionar. A figura fixa a cadeia e os pressupostos; a Tabela~\ref{tab:matriz-rastreabilidade} responde, cláusula a cláusula, ao que é exigido e ao que fica por especificar.

%% 2026-09-19: figura movida para ficheiro proprio e redesenhada no
%% estilo partilhado. Versao anterior em _revisao/2026-09-19f-cap2-antes-fig21.tex.
%% \newpage (nao \pagebreak): fecha a pagina no paragrafo que apresenta a
%% figura sem esticar o que fica acima, para que a figura abra a pagina
%% seguinte e preceda o percurso pelos elos. Ver o cabecalho do ficheiro
%% da figura.
\newpage
\input{5-Figures/tikz/f02a-cadeia-normativa}

Percorramos os elos onde o texto normativo é mais revelador.

\paragraph{Variáveis relevantes (elo 3).} A ISO~50006 dedica ao problema orientação prática: recomenda gráficos de tendência e diagramas X--Y entre consumo e variável candidata, e observa que \enquote{em muitos casos, uma relação linear simples é adequada para determinar a relevância} (\S4.2.4). O critério de significância, porém, \enquote{é determinado pela organização} (ISO~50001, \S3.4.9, Nota~1) --- a norma diz \emph{como olhar}, mas não fixa nenhum limiar de decisão \alterado{nem}, sobretudo, nenhum
teste de \emph{suficiência} \alterado{do conjunto escolhido}: nada obriga a organização a demonstrar que o conjunto de variáveis escolhido esgota a variação sistemática do consumo. Nenhuma cláusula distingue a escolha de uma única variável --- a carga processada, no caso em estudo --- da escolha de cinco.

\paragraph{Tipo de EnPI (elo 4).} A Tabela~2 da ISO~50006 é o artefacto mais interessante de toda a família para o argumento desta dissertação, porque a própria norma enumera, na coluna de observações, as fragilidades de cada tipo. Para o tipo \enquote{valor medido}: não tem em conta os efeitos das variáveis relevantes, \enquote{dando resultados enganadores na maioria das aplicações}. Para o rácio: não considera a carga de base nem efeitos não lineares, sendo \enquote{enganador em instalações com carga de base elevada}. Para o modelo estatístico: \enquote{pode ser impreciso se não for confirmado por testes estatísticos} e \enquote{os modelos devem ser mantidos para assegurar resultados válidos} (\S4.3.3, Tabela~2). E, na mesma linha dessa tabela, uma observação que convém ler devagar: \enquote{pode não ser claro se algum erro residual se deve a erro de modelação ou a falta de controlo sobre o consumo de energia}. É o problema desta dissertação enunciado pela própria norma --- o resíduo de uma \emph{baseline} não separa o que o modelo não sabe do que a unidade fez mal --- e enunciado \alterado{sem que lhe corresponda procedimento para separar as
duas causas.}\footnote{Traduções do autor a partir do original inglês; as passagens críticas são reproduzidas no original em nota nas ocorrências seguintes.} A norma conhece, portanto, os modos de falha --- enuncia-os --- mas não converte nenhum deles em requisito verificável: \alterado{não diz \emph{que} testes, \emph{com que limiares} nem \emph{com
que frequência}; e a resposta que prevê --- ajustar o EnB ou determinar um
novo modelo (\S4.4.3) --- não fica dependente de nenhum critério
declarado.} A Tabela~\ref{tab:tipologia-enpi} condensa esta tipologia e localiza o método em produção na refinaria --- regressão linear univariada por vetor energético, $E = a \cdot Q + b$ --- na classe \enquote{modelo estatístico}, herdando por escrito os \emph{caveats} que a própria norma lhe associa.
...
\input{5-Figures/tikz/f02-enpi-enb-meta}

\section{A assimetria normativa}
\label{sec:assimetria}
%% 2026-09-19: caixa C2-05 resolvida e retirada. A repeticao entre 2.2.2,
%% 2.2.3 e 2.3.1 foi cortada (2.2.3 fica com a delimitacao do que se critica,
%% 2.3.1 nomeia a assimetria sem refazer o percurso e herda a frase de
%% remate); a matriz da Tabela 2.3 e agora a unica lista clausula a clausula
%% e a sua legenda declara que a coluna dos pressupostos e reconstrucao desta
%% dissertacao; a 2.3.1 di-lo tambem no corpo; e a politica de citacao passou
%% a estar declarada no paragrafo "Nomenclatura e citacoes" da 2.1, com duas
%% transcricoes decorativas convertidas em parafrase. Versao anterior em
%% _revisao/2026-09-19b-cap2-antes.tex.

\subsection{Formulação}
\label{sec:assimetria-formulacao}

Chamamos assimetria normativa ao desequilíbrio entre a densidade
prescritiva das normas quanto à \emph{existência} dos instrumentos de
medição do desempenho energético e o seu silêncio quanto \alterado{aos critérios pelos quais a
\emph{validade}} desses instrumentos \alterado{se decide}. A Secção~\ref{sec:enb-cadeia} percorreu-o elo a elo e a
Tabela~\ref{tab:matriz-rastreabilidade} fixa-o cláusula a cláusula; não se
refaz aqui o percurso. O que interessa reter é a forma: a família nomeia os
instrumentos estatísticos com que a validade poderia ser aferida e em nenhum
ponto converte a aferição em requisito, fixa limiares de aceitação, exige a
verificação dos pressupostos dos modelos \alterado{ou fixa um critério de desempenho para a deteção da invalidade.
Nomear verificações, a ISO~50006 nomeia: a \S4.6 enumera testes para decidir
se o EnPI e o EnB continuam apropriados, entre eles comparar os valores das
variáveis relevantes do período de referência com as condições do período de
reporte, para ver se caem num intervalo estatístico válido. O que fica por
dizer é o que torna esse intervalo válido, com que cobertura e com que taxa
de falsos sinais --- a diferença entre nomear uma verificação e especificar
um detetor.} Daqui vem a
consequência que o resto do capítulo persegue: a ausência de uma barreira
normativa não é evidência de validade estatística, e \alterado{as verificações que as normas preveem não trazem o critério de
desempenho que permitiria distinguir uma coisa da outra.} O
Capítulo~\ref{cha:diagnostico} examina essa situação nos vetores energéticos
da unidade em estudo (Secção~\ref{sec:caso-estudo}).

A assimetria tem uma segunda face, menos evidente: as normas embutem pressupostos estatísticos que nunca enunciam. A comparação \enquote{em condições equivalentes} pressupõe a suficiência das variáveis relevantes; a extrapolação do modelo de \emph{baseline} para o período de reporte pressupõe a estabilidade estrutural da relação; \alterado{escolher} doze meses \alterado{de referência} pressupõe que um ciclo anual é representativo; os gatilhos de revisão pressupõem que a mudança é observável por quem opera. O Anexo~D da ISO~50006 é o lugar onde o pressuposto da transportabilidade mais trabalha e menos se declara: o modelo estimado no período de referência é usado para prever o período de reporte, e a diferença entre o previsto e o observado \enquote{indicará se ocorreu uma melhoria de desempenho energético} (\S D.1). O anexo ressalva que os valores previstos e os observados não coincidem exatamente; não ressalva que a relação estimada à esquerda possa ter deixado de valer à direita. Nenhum destes pressupostos é neutro num processo contínuo sujeito a
\emph{fouling}, desativação de catalisador e deslocamento gradual do espaço
operacional --- a especificidade desenvolvida na
Secção~\ref{sec:processos-continuos}. Diga-se com clareza: os
pressupostos são leitura desta dissertação, não texto normativo. Nenhuma das
três normas os enuncia como pressupostos, e a cadeia de medição da
Figura~\ref{fig:cadeia-normativa} é um instrumento de análise construído
aqui, não uma paráfrase das cláusulas.

\subsection{Matriz de rastreabilidade: cláusula, silêncio, pressuposto, teste}
\label{sec:assimetria-matriz}

A Tabela~\ref{tab:matriz-rastreabilidade} formaliza a análise: para cada cláusula relevante, o que é exigido ou sugerido, o que fica por especificar, o pressuposto tácito que a cláusula importa, e o \emph{endpoint} do protocolo de diagnóstico (Secção~\ref{sec:protocolo-baselines}) que o operacionaliza. É esta última coluna que \alterado{liga cada leitura da norma ao teste que
a examina no} Capítulo~\ref{cha:diagnostico}.

\begin{table}[htbp]
  \centering\footnotesize
  \caption[Matriz de rastreabilidade cláusula--silêncio--pressuposto--teste]{Matriz de rastreabilidade: cláusula normativa, silêncio, pressuposto tácito
  e \emph{endpoint} de diagnóstico que o testa (E1--E6 e bateria descritiva D;
  ver Secção~\ref{sec:protocolo-baselines}). As duas primeiras colunas
  reportam o texto das normas; a coluna dos pressupostos é reconstrução desta
  dissertação, que nenhuma das normas enuncia.}
  ...
  \begin{tabularx}{\textwidth}{@{}c >{\raggedright\arraybackslash}p{2.1cm} X X X c@{}}
    ...
    2 & 50001 \S6.3~c)\,1), \S3.4.9~N1; 50006 \S4.2.4 & Determinar variáveis relevantes (métodos gráficos) & Limiar de significância; teste de \emph{suficiência} do conjunto & As variáveis descrevem o estado operacional & E6 \\
    ...
    7 & 50001 \S6.5~a)--c); 50006 \S4.6, \S3.1~N3 & Rever o EnB (3 gatilhos; \emph{reset} periódico sancionado) & \alterado{Critério de desempenho da deteção}; gatilho~a) circular & Mudanças invalidantes são conhecidas ou visíveis & E5 \\
    8 & 50015 \S7 + Nota & Identificar e quantificar incerteza ($t$, $R^2$, $p$, limites de predição nomeados) & \alterado{Obrigatoriedade; limiares de aceitação} & A incerteza declarada é gerível caso a caso & E4 \\
    ...
    10 & 50006 \S4.2.6.4 & Rever qualidade dos dados; examinar \emph{outliers} & \alterado{Número de desvios-padrão que exclui}; efeito sobre $\sigma$ & \emph{Outliers} são raros e identificáveis & D \\
    \bottomrule
  \end{tabularx}
\end{table}

A leitura vertical da coluna dos pressupostos produz a lista compacta que estrutura o resto da dissertação: \textbf{suficiência} (as variáveis descrevem o estado), \textbf{forma} (a classe de modelo captura a relação), \textbf{representatividade} (o período de treino antecipa o futuro), \textbf{transportabilidade} \alterado{(a relação vale na aplicação, salvo
pela mudança de desempenho que se procura)}, \textbf{calibração} (os desvios têm escala conhecida) e \textbf{detetabilidade} (a invalidação é observável). Os resultados do Capítulo~\ref{cha:diagnostico} organizam-se por estes pressupostos, com uma ressalva: nem todos os testaram com o mesmo alcance, e a detetabilidade real ficou por estimar.

% 2026-09-19: titulo alargado (era "... com o IPMVP e a ASHRAE Guideline 14")
\subsection[O silêncio como escolha: comparação com o IPMVP, a ASHRAE Guideline 14 e os quadros da indústria de processo]{O silêncio como escolha: comparação com o IPMVP, a ASHRAE Guideline 14 e os quadros aplicáveis à indústria de processo}
\label{sec:assimetria-comparacao}

Poderia argumentar-se que o silêncio quantitativo das normas ISO é inevitável --- que critérios de validação universais não podem ser fixados para organizações de todos os tipos e dimensões; é, de resto, a justificação implícita do \enquote{conforme definido pela organização} que percorre os três documentos. A comparação com os protocolos de M\&V do domínio dos edifícios desmente a
inevitabilidade\novo{, e a comparação com os quadros que vinculam ou
certificam a própria indústria de processo fecha-lhe a saída restante}. São
\novo{quatro} frentes distintas.
...
\novo{Quarta frente, sem transposição entre setores: \emph{critérios
quantitativos em quadros aplicáveis a processo contínuo}. A objeção de que os
limiares anteriores nasceram no domínio dos edifícios --- válida quanto aos
números --- não salva a tese da
inevitabilidade, porque há dois quadros fora desse domínio que os fixam, e um
deles é direito vinculativo sobre a instalação em estudo.}

\novo{O Regulamento de Execução (UE) 2018/2066, que rege a monitorização e a
comunicação de emissões no regime europeu de comércio de licenças e se aplica
à refinação de petróleo sem limiar de
capacidade~\cite[Anexo~I]{ETS_Directive_2003}, define a incerteza como um
intervalo de confiança a $95\,\%$ em torno do valor médio (artigo~3.º, n.º~6)
e fixa, no Anexo~II, incertezas máximas admissíveis por nível de exatidão, com
um bloco dedicado à refinação: de $\pm7{,}5\,\%$ (nível~1) a
$\pm1{,}5\,\%$ (nível~4) nos fluxos-fonte de combustível, e até
$\pm2{,}5\,\%$ na regeneração de catalisador de \emph{cracking}. O
artigo~26.º obriga a aplicar o nível mais elevado, com descida só mediante
prova de inviabilidade técnica ou de custos desproporcionados, e o
artigo~28.º, n.º~2, exige que a avaliação de incerteza seja entregue com o
plano de monitorização~\cite{MRR_2018}. A verificação independente tem, ela própria,
limiar de materialidade declarado~\cite[artigo~23.º]{AVR_2018}.}

\novo{A comparação que daqui resulta é interna à instalação. Na mesma
refinaria e a partir, em boa medida, das mesmas medições de combustível, o número de carbono tem
incerteza máxima declarada, nível de confiança fixado, avaliação documentada e
materialidade de verificação; o número de energia não tem nenhuma das quatro. A assimetria não opõe edifícios a processo contínuo: opõe
dois regimes que coabitam na mesma instalação, sobre a mesma medição, e que
fizeram escolhas opostas quanto a fixar critérios.}

\novo{O segundo quadro é um protocolo de M\&V industrial construído sobre a
própria ISO~50001. O programa \emph{Superior Energy Performance} do
Departamento de Energia norte-americano exige, para que um modelo de ajuste
seja considerado estatisticamente válido, que o teste~$F$ do modelo tenha
$p \leq 0{,}10$, que todas as variáveis relevantes incluídas tenham
$p \leq 0{,}20$, que pelo menos uma tenha $p \leq 0{,}10$, que
$R^2 \geq 0{,}50$, e que a seleção de variáveis e os coeficientes sejam
coerentes com a compreensão física do consumo da
instalação~\cite[p.~7]{Therkelsen2016}. É invocado por sete publicações do corpus da
revisão (Capítulo~\ref{cha:revisao}). Os números são de ajuste e de
significância, não de desempenho fora do treino, de calibração ou de deteção,
que é o que o Capítulo~\ref{cha:metodos} adota. O
ponto é que um programa de âmbito geral, dirigido à indústria e assente na
ISO~50001, \emph{consegue} fixá-los. A camada de critérios existe; está fora
da família ISO, e teve de ser acrescentada por quem quis auditar desempenho.}

Independentemente do juízo sobre estes limiares em concreto --- a literatura discute a sua suficiência estatística, e o Capítulo~\ref{cha:metodos} adotará critérios de outra natureza, sobre desempenho fora do treino, calibração e deteção, que não se comparam com estes limiares de ajuste --- a sua simples existência demonstra que protocolos com ambição de aplicação geral \emph{conseguem} fixar critérios quantitativos de aceitação de modelos, regras de tratamento de dados e tetos de incerteza. A família ISO~50001 \alterado{não o faz}\novo{, nas três normas
analisadas}.\footnote{\novo{A ISO~50047:2016, da mesma série, não foi
consultada: o único excerto público é o do projeto que lhe deu origem
(ISO/DIS~17747), até à cláusula~5.1.1, e o índice anuncia uma cláusula~7.4,
\enquote{Acceptable Uncertainty Criteria}, que não foi possível verificar
(Secção~\ref{sec:limitacoes}). No excerto acessível o registo é de
recomendação e o âmbito remete os métodos de M\&V para a
ISO~50015~\cite{ISO50047_2016}.}} A
Tabela~\ref{tab:comparacao-frameworks} sintetiza a comparação\novo{; a
Tabela~\ref{tab:comparadores-industriais} estende-a aos quadros aplicáveis a
processo contínuo}.

% 2026-09-19: as duas tabelas de comparacao formam par e o texto anuncia-as
% juntas. Como floats ([htbp], [H], [p]) nunca ficavam adjacentes sem custo --
% adiamento para depois das notas que as referem, buraco a meio da pagina por
% \flushbottom, ou corte de paragrafos a meio. Passam a xltabular (nao
% flutuam, partem-se entre paginas), precedidas de \newpage: a pagina anterior
% fecha com o paragrafo que as anuncia e a nota 4, e a seguinte abre com a 2.4
% seguida da 2.5. O paragrafo da clarificacao passou para depois das tabelas.
\newpage

{\footnotesize
\ntzebra
\begin{xltabular}{\textwidth}{@{}>{\raggedright\arraybackslash}p{3.2cm} X X X X@{}}
\caption[Comparação de quadros normativos quanto a requisitos de validação]{Comparação de quadros normativos e protocolos de M\&V quanto a requisitos de validação de modelos de \emph{baseline}. ASHRAE Guideline~14 na edição de 2014; o conteúdo da coluna IPMVP é relatado por \textcite{RuizBandera2017}, sem consulta da edição corrente do protocolo.}\label{tab:comparacao-frameworks}\\
\hiderowcolors
\toprule
...
\endhead
\midrule
\rowcolor{white}\multicolumn{5}{r}{\emph{continua na página seguinte}}\\
\endfoot
\bottomrule
\endlastfoot
\showrowcolors
    ...
    Tratamento de exclusões e \emph{outliers} & --- & \alterado{Investigar, justificar e evitar viés, sem limiar} (50006 \S4.2.6.4) & Justificação exigida & Proibidas na via prescritiva; $\leq 25\,\%$, documentadas, na simulação \\
    Verificação dos pressupostos dos testes & --- & --- & Parcial & Parcial \\
    Incerteza reportada com o resultado & --- & \alterado{Reporte recomendado (50015 \S4.2); quantificação}
    \enquote{na medida do praticável} (\S7) & Sim (Apêndice~B) & Sim, com teto: $<50\,\%$ das poupanças a $68\,\%$ de confiança \\
...
{\footnotesize
% espaco antes apertado (local a este grupo): a 2.5 arranca na mesma pagina da
% 2.4 e parte-se para a seguinte, em vez de deixar um vazio no fundo da pagina.
\setlength{\LTpre}{\smallskipamount}
\ntzebra
\begin{xltabular}{\textwidth}{@{}>{\raggedright\arraybackslash}p{3.1cm} X X X@{}}
\caption[Quadros aplicáveis a processo contínuo quanto a critérios quantitativos]{\novo{Quadros aplicáveis a processo contínuo, quanto a critérios quantitativos. Nenhum é um protocolo de validação de modelos de \emph{baseline} energética.}}\label{tab:comparadores-industriais}\\
\hiderowcolors
\toprule
\textbf{Quadro} & \textbf{Objeto do critério} & \textbf{Critério quantitativo} & \textbf{Estatuto} \\
\midrule
\endfirsthead
\toprule
\textbf{Quadro} & \textbf{Objeto do critério} & \textbf{Critério quantitativo} & \textbf{Estatuto} \\
...
\endlastfoot
\showrowcolors
    \novo{Reg. (UE) 2018/2066 e 2018/2067 (CELE)} & \novo{Incerteza da medição de fluxos-fonte de GEE} & \novo{Incertezas máximas por nível ($\pm1{,}5$ a $\pm10\,\%$) a $95\,\%$ de confiança; materialidade de $5\,\%$ ou $2\,\%$} & \novo{Direito da União, vinculativo; aplica-se à instalação em estudo} \\
    \novo{SEP M\&V Protocol (DOE)} & \novo{Validade estatística do modelo de ajuste de uma instalação industrial} & \novo{$F$ do modelo com $p \leq 0{,}10$; variáveis com $p \leq 0{,}20$, uma delas com $p \leq 0{,}10$; $R^2 \geq 0{,}50$} & \novo{Programa voluntário de certificação, construído sobre a ISO~50001} \\
    \novo{\emph{Benchmarking} energético setorial} & \novo{Desempenho relativo face a pares, contra energia padrão de coeficientes fixos} & \novo{Nenhum sobre o modelo; quartil de desempenho no uso certificativo} & \novo{Estudo comercial; coeficientes proprietários, não reestimados pela instalação} \\
    \novo{ISO 50001 / 50006 / 50015} & \novo{EnB e EnPI definidos pela organização} & \novo{Nenhum} & \novo{Requisitos (50001) e orientação (50006 e 50015)} \\
\end{xltabular}
}

\novo{Uma clarificação, para prevenir a objeção óbvia. As
metodologias de \emph{benchmarking} energético do setor da refinação,
incluindo a que a instalação em estudo acompanha
(Secção~\ref{sec:caso-estudo}), não são protocolos de M\&V e não preenchem
esta lacuna. Comparam o consumo de cada unidade com uma energia padrão de
coeficientes fixos, estimados pelo operador do estudo sobre a sua própria base
de dados setorial e não reestimados pela instalação; respondem à pergunta
\enquote{como me comparo com os meus pares}, não \enquote{o modelo com que
sigo esta unidade continua válido}. O controlo de qualidade que lhes está
associado incide sobre os dados de entrada --- coerência interna, balanços de
massa e energia, desvios face ao reportado no passado --- e não sobre a
validade do modelo que produz o padrão. Os critérios quantitativos que
acompanham o seu uso certificativo, como o quartil superior de desempenho
exigido para a certificação industrial ENERGY~STAR de refinarias, em que o
engenheiro certificador verifica dados, fronteira e período e não refaz
análise~\cite[\S4]{EnergyStarPE_2013}, são critérios de \emph{desempenho}, não
de validade do modelo. A distinção entre as duas coisas atravessa todo este
capítulo.}

Três notas de rigor. Primeira: os limiares ASHRAE/IPMVP/FEMP foram desenvolvidos para modelos de energia de edifícios, com sazonalidade meteorológica dominante; a sua transposição direta para processo contínuo não é defendida aqui --- o que se importa é o \emph{princípio} de critérios de aceitação explícitos, não os números. \novo{Os limiares do Regulamento~2018/2066 não têm esse problema de domínio, mas incidem sobre a incerteza da medição de um fluxo-fonte e não sobre a validade de um modelo de consumo: servem para mostrar que fixar critérios é praticável no mesmo processo e sobre os mesmos instrumentos, não para serem importados como critérios de modelo.} Segunda: mesmo estes protocolos são imperfeitos --- \textcite{RuizBandera2017} documentam inconsistências de definição entre eles e entre as suas sucessivas edições (a confusão sistemática entre MBE e NMBE, fórmulas divergentes, limiares que mudam de versão para versão), \alterado{e a verificação dos pressupostos distribucionais dos seus
próprios índices é, quando existe, parcial
(Tabela~\ref{tab:comparacao-frameworks}).} O \emph{gap} identificado no Capítulo~\ref{cha:revisao} \alterado{não é,
portanto, exclusivo da família ISO}; a família ISO é \alterado{o caso extremo, por nomear índices sem fixar nenhum}.

\novo{Terceira: a existência de critérios não elimina o espaço de escolha que
este capítulo identifica. O próprio protocolo SEP não fixa qual dos quatro
métodos de normalização deve ser usado, e \textcite{ChenTherkelsen2019}
mostram, sobre um único conjunto de dados industriais, que as poupanças
calculadas em conformidade com o protocolo variam entre $-1\,091$ e
$142\,248$~MMBtu, e o indicador normalizado entre $0{,}93$ e $1{,}00$,
consoante o método escolhido. Fixar limiares de aceitação do modelo é condição
necessária e não suficiente; a pergunta que o resto da dissertação persegue
não é apenas se existem critérios, mas se os critérios existentes interrogam
os pressupostos de que a validade depende.}

\paragraph{Nota sobre as edições analisadas.} A presente análise reporta-se à ISO~50006:2014 e à ISO~50015:2014, as edições a que houve acesso. \alterado{Não se confirmou qual a edição
enquadra o método de \emph{baseline} em produção na instalação em estudo.} Existe uma segunda edição da ISO~50006, de 2023, que substituiu a de 2014, harmonizada com a ISO~50001:2018, que introduz, entre outras alterações, uma cláusula dedicada à incerteza de modelo (\S8.2)~\cite{ISO50006_2023}; não foi possível consultá-la no âmbito deste trabalho. Regista-se como limitação (Secção~\ref{sec:limitacoes}): as afirmações desta secção sobre o silêncio normativo estão verificadas contra as edições de 2014, e a possibilidade de a edição de 2023 mitigar parcialmente algum dos silêncios identificados não pôde ser excluída nem confirmada. Assinale-se, porém, que a ISO~50006:2023 mantém o estatuto de norma de orientação, pelo que o argumento central --- a ausência de \emph{requisitos} de validação no único documento certificável da família, a ISO~50001 --- não depende dessa edição.

\subsection{Síntese}
\label{sec:assimetria-sintese}

O quadro normativo obriga a medir e a demonstrar melhoria, sugere sem obrigar os instrumentos de validação, e é silencioso sobre os pressupostos estatísticos de que a validade desses instrumentos depende. \alterado{Num processo estável, o silêncio custa menos.} A questão a que o resto da dissertação responde empiricamente é o que acontece quando o processo não é estacionário --- quando o \emph{fouling}, as transições de consumidores e o deslocamento do espaço operacional fazem parte da física da unidade (Secção~\ref{sec:processos-continuos}) --- e o espaço de implementação permitido pela norma é preenchido pela escolha mais simples que nele cabe: uma regressão univariada estática, com bandas de $\pm 2\sigma$,
\alterado{estimada por ano civil} (Secção~\ref{sec:caso-estudo}).

\section{Especificidade dos processos contínuos}
\label{sec:processos-continuos}

\alterado{Num edifício, as variáveis que explicam o consumo são sobretudo
exteriores ao sistema e periódicas --- a temperatura exterior, a ocupação, o
calendário --- e o que se move é o valor delas.} Num processo contínuo de refinação, a relação move-se
...
Os depósitos que o crude forma nas superfícies de transferência de calor reduzem o calor
...
corte junto a ela, o que faz com que a mesma bateria apresente diferentes ritmos de incrustação consoante o caudal e o crude~\cite{Wilson2017}. O depósito envelhece e
endurece, e a limpeza, que interrompe a transferência de calor e tem custo, é agendada num
...
específico do forno, assume um padrão em dente de serra: subida gradual ao longo de semanas
...
modelo reajustado todos os anos \alterado{acomoda-a nos parâmetros da nova reta}.
...
este mecanismo e o anterior aparecem confundidos nos dados.
...
medição, e \alterado{nenhuma se distingue das outras} num modelo que não receba o
registo do evento. A distinção importa: uma melhoria real, uma limpeza e um instrumento
...
identificadas para estas unidades~\cite{WorrellGalitsky2005}. São também, pelas razões desta secção, o sítio onde \alterado{os mecanismos
que comprometem uma referência estática se acumulam}. A Tabela~\ref{tab:mecanismos-pressupostos} fecha a secção: para cada
...
A Secção~\ref{sec:corpus-b} \alterado{mostra que a literatura sobre indicadores e referências raramente
nomeia a causa física do que corre mal ao modelo: em 89 das 130 ocorrências
de efeito extraídas do Corpus~B não se nomeia nenhuma.} É uma razão adicional para as fixar
aqui, antes dos dados.

% 2026-09-20: passou de float ([htbp]) a xltabular, como as Tabelas 1.1,
% 2.1, 2.4 e 2.5, e mudou de sitio: fecha agora mesmo a seccao, a seguir ao
% ultimo paragrafo e antes da 2.5, como o texto anuncia. Nao flutua e
% parte-se entre paginas com cabecalho repetido.
{\footnotesize
\ntzebra
\begin{xltabular}{\textwidth}{@{}>{\raggedright\arraybackslash}p{2.3cm} >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}p{2.1cm}@{}}
\caption[Mecanismos de processo contínuo e pressupostos que violam]{Mecanismos de não estacionariedade em processo contínuo, o que deles se observa e o pressuposto tácito que violam. CDU: presente na unidade estudada. Os pressupostos são os da Secção~\ref{sec:assimetria-matriz}.}\label{tab:mecanismos-pressupostos}\\
\hiderowcolors
\toprule
...
\paragraph{\novo{A fronteira do indicador não é a da coluna.}}
\novo{Antes dos vetores, uma precisão que condiciona tudo o que se segue. A
unidade a que o modelo de dados chama destilação atmosférica não é a coluna:
é um bloco de contabilização que agrega, além dela, as secções auxiliares
declaradas na sua composição --- o tratamento de gases com aminas, as
unidades de adoçamento e o fracionamento de gases. O numerador do indicador
soma-as todas; o denominador é a carga de crude à coluna. Nove dos dezasseis
blocos da plataforma são compostos deste modo
(Secção~\ref{sec:validacao-dados}).}

\novo{A consequência é anterior a qualquer discussão de covariáveis. O
consumo das secções auxiliares responde à produção de gases, à carga de
enxofre e ao perfil de produtos leves, não à tonelagem de crude: a
suficiência da carga como única variável explicativa fica comprometida pelo
desenho da fronteira, e não por uma insuficiência física da relação
consumo--carga. É, de todos os mecanismos deste capítulo, o único que
nenhuma reestimação da reta corrige, porque não está no ajuste --- está na
definição do indicador, no elo~1 da cadeia da Secção~\ref{sec:enb-cadeia}
(ISO~50006, \S4.2.2). O Capítulo~\ref{cha:diagnostico} retoma-o como
explicação concorrente da variância residual.}

\paragraph{Os cinco vetores.} O consumo \novo{assim delimitado} é
...
assumem resíduos com escala conhecida; \alterado{e a referência é uma reta por ano civil, sendo o uso avaliado
nesta dissertação o de ler cada ano contra a reta do anterior}. É este o objeto que o resto da dissertação examina: replicado de
...
\novo{Confirmar em particular a menção, agora explícita, a que a fronteira do
indicador agrega à coluna o tratamento de gases com aminas, o adoçamento e o
fracionamento de gases: nomeiam-se funções genéricas de qualquer bloco de
destilação, não códigos internos nem configuração.}
