...
Independentemente do juízo sobre estes limiares em concreto --- a literatura discute a sua suficiência estatística, e o Capítulo~\ref{cha:metodos} adotará critérios de outra natureza, sobre desempenho fora do treino, calibração e deteção, que não se comparam com estes limiares de ajuste --- a sua simples existência demonstra que protocolos com ambição de aplicação geral \emph{conseguem} fixar critérios quantitativos de aceitação de modelos, regras de tratamento de dados e tetos de incerteza. A família ISO~50001 \alterado{não o faz}. A Tabela~\ref{tab:comparacao-frameworks} sintetiza a comparação.
