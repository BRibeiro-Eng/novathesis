...
% Rascunho transversal, 2026-09-15. Activado no build a 2026-09-16; o esqueleto anterior está em
% 3-BackMatter/_arquivo/appendix-G-matematica-esqueleto-20260813.tex. MAP liga cada passagem ao inventário editorial.
...
\matref{As convenções aqui declaradas foram confrontadas com o modelo da plataforma e com o código de análise nas passagens que o texto identifica. Essa consulta esclarece convenções; não certifica a correspondência de cada resultado publicado com o lote que o produziu, e a distinção é mantida explícita ao longo do apêndice. Onde uma convenção não pôde ser confirmada na fonte, o limite é declarado em vez de omitido; as formulações ainda não executadas são apresentadas como propostas e identificadas como tal.}
...
\matref{A sequência efetivamente aplicada pela plataforma foi confrontada com a partição que estima as retas de referência. Os filtros incidem sobre células vetor--dia e a sua precedência é a seguinte: pertença ao conjunto declarado de vetores; energia diária não vazia; energia diária diferente de zero, exclusão introduzida para remover zeros produzidos pelo canal de balanço; data anterior ao dia corrente, por o dia em curso ser parcial; existência de carga no nível de agregação da unidade e de limiar declarado para essa unidade; e carga não inferior a esse limiar, com desigualdade inclusiva. Só depois destes filtros é construída a linha do agregado, somando por dia os vetores do perímetro orçamentável que sobreviveram; um dia em que falte um vetor produz, por isso, um agregado formado pelos restantes, e não uma ausência --- o total diário do agregado não é condicional à presença de todos os vetores. O agrupamento final é por unidade, vetor e ano civil. A publicação das contagens após cada filtro, distinguindo dias únicos de células vetor--dia, não consta dos artefactos consultados: as frações de exclusão citadas na dissertação referem-se à população final e não decompõem os motivos.}
...
\matref{A população que estes filtros deixam, e a alocação mensal da
eletricidade a que a equação~\ref{eq:mat-alocacao-mensal} se refere, estão
representadas na Figura~\ref{fig:populacao-analitica}, no capítulo de
métodos, com as contagens da extração congelada.}
...
% MAP: M02.01 M02.02
A carga só pode ser interpretada depois de fixada a fronteira do processo\matref{~\parencite{FelderRousseau2015}}. Para uma unidade, o balanço material integrado num período é
\begin{equation}
...
Desprezando variações de energia cinética e potencial, uma forma do balanço energético é\matref{~\parencite{FelderRousseau2015}}
\begin{equation}
...
\matref{A fronteira concreta da unidade avaliada --- que correntes entram em
$Q_t$ e em cada $E_{v,t}$ --- é descrita no
Capítulo~\ref{cha:enquadramento} e fixada pela tabela de mapeamento do
Capítulo~\ref{cha:metodos}, que é onde a atribuição de cada corrente fica
registada linha a linha. Um diagrama de fluxos com larguras proporcionais
exigiria um balanço reconciliado num período comum, que a plataforma não
produz: a harmonização da equação~\ref{eq:mat-fontes} aplica precedências
declaradas e não resolve um balanço com incertezas de medição. Representar
caudais a escala sem esse balanço sugeriria uma conservação que os dados
não sustentam.}

\subsection{Conversão em gás natural equivalente}
\label{sec:mat-gne}
% MAP: M02.03 M02.05 M02.06
\matref{A conversão a um combustível de referência é uma convenção contabilística interna, e não uma medida de eficiência termodinâmica; a sua base está declarada na tabela de parâmetros globais do modelo, nos termos indicados no final desta secção.} Para uma massa $m_j$ de combustível com poder calorífico inferior $\mathrm{PCI}_j$, a massa equivalente de gás natural é
\begin{equation}
...
\matref{Os valores foram confirmados na tabela de parâmetros globais, que regista para cada um a sua origem. O poder calorífico de referência $\mathrm{PCI}_{\mathrm{ref}}=11\,820$ kcal/kg e o fator de eficiência $0{,}57$ são declarados como normas internas da empresa; o fator $860{,}4$ é declarado como equivalência física. Os equivalentes entálpicos são atribuídos na própria tabela --- e não pela ordem em que aparecem --- a $698$ kcal/kg no nível de $3{,}5$ bar, $738$ kcal/kg no de $10{,}5$ bar e $758$ kcal/kg no de $24$ bar, tendo por fonte tabelas de vapor saturado. Subsistem duas divergências que importa declarar. Os níveis designados no corpo da dissertação por $3$ e $10$ bar correspondem, na tabela de parâmetros, a $3{,}5$ e $10{,}5$ bar, pelo que os nomes são rótulos de nível e não pressões de operação. E o fator $860{,}4$ é registado com a unidade kcal/MWh, ao passo que a sua descrição e a aritmética da equação~\ref{eq:mat-gne-eletricidade} exigem Mcal/MWh; a unidade inscrita na tabela está, nesse campo, errada por um fator de mil. Nenhum dos parâmetros tem data de vigência registada: a tabela documenta a base de cada valor, não o período em que se aplica.}
...
O consumo específico é o indicador de desempenho energético que a norma tipifica\matref{~\parencite{ISO50006_2014,ISO50047_2016}}. O de um período $\mathcal P$ é a razão de totais calculados nos mesmos dias:
\begin{equation}
...
\matref{O ramo de cálculo da unidade de destilação --- uma função por partes de grau API, rendimento, temperatura de vaporização e logaritmo da pressão, com recusa explícita perante valores em falta ou fora dos limites numéricos --- não é reproduzido nesta dissertação, nem nos seus coeficientes nem na sua forma. A metodologia de comparação em que assenta é material proprietário do fornecedor do estudo e o adendo contratual aplicável proíbe expressamente a sua reprodução; nenhum coeficiente, energia padrão ou fórmula dessa correlação consta deste texto, da bibliografia ou dos ficheiros versionados. A equação~\ref{eq:mat-eii} define, por isso, apenas a estrutura contabilística do índice, e $F_j$ permanece uma função documentada no dicionário de dados interno. Daqui decorre um limite que se declara: os valores do índice apresentados na dissertação são lidos tal como a plataforma os produz, sem verificação independente da correlação que os sustenta.}

\subsection{Emissões calculadas}
\label{sec:mat-emissoes}
% MAP: M02.13
Com fatores $f_v$ em t CO$_2$/t GNE\matref{~---~cuja origem regulamentar,
para uma instalação abrangida pelo comércio de licenças, é o regulamento de
monitorização e comunicação~\parencite{MRR_2018,ETS_Directive_2003}~---}, a
conversão contabilística é
\begin{equation}
...
\matref{Os quatro percursos de conversão --- combustível, vapor,
eletricidade e emissões --- estão nas equações~\ref{eq:mat-gne-combustivel}
a~\ref{eq:mat-emissoes}, e o fator, a unidade e a origem de cada parâmetro
ficam declarados no final da Secção~\ref{sec:mat-gne}. A dependência do
consumo específico com a carga é a própria
equação~\ref{eq:mat-racio-carga}: um termo constante repartido por mais
produção. Uma figura que repetisse estas identidades acrescentaria uma
página sem acrescentar leitura.}
...
\subsection{Matrizes de incidência e redes de citação\matref{~\parencite{Kessler1963}}}
\label{sec:mat-redes}
% MAP: M03.05 M03.06
...
\subsection{Comunidades e posição dos nós\matref{~\parencite{NewmanGirvan2004,Blondel2008}}}
...
\subsection{TF--IDF e tópicos exploratórios\matref{~\parencite{SparckJones1972}}}
...
\matref{As contagens e os mapas efetivamente usados na revisão estão no
Capítulo~\ref{cha:revisao} --- a cobertura por campo na
Figura~\ref{fig:cobertura-campos} e o mapa de casos na
Figura~\ref{fig:mapa-casos} --- e nenhum deles é uma rede de coocorrência.
A opção foi deliberada: com o denominador e os limiares deste corpus, uma
rede não separa o que as barras e as interseções exatas já separam. As
expressões desta secção descrevem, por isso, a construção de uma rede para
quem queira reproduzi-la, não um resultado apresentado.}
...
\matref{As taxas de erro por campo e os respetivos denominadores estão na
Figura~\ref{fig:cobertura-campos} do Capítulo~\ref{cha:revisao}, com $n_v$
declarado em cada ponto. O limite superior unilateral para zero erros é a
forma fechada da Secção~\ref{sec:mat-clopper-pearson}, e para a amostra
auditada dá diretamente o valor citado nesse capítulo; é uma identidade de
uma linha, que uma curva ilustraria sem tornar mais exata. O que importa
conservar não é a figura mas a separação dos denominadores: as decisões
censitadas não entram na fração amostral.}
...
Para um vetor energético e um período de treino, a baseline linear simples
é\matref{~\parencite{DraperSmith1998}}
\begin{equation}
...
\matref{A distinção entre ajuste calculável e ajuste admissível existe na plataforma, mas reside em dois lugares distintos e com valores distintos, o que importa declarar. A partição que estima as retas calcula declive, intercepto, escala e $R^2$ a partir de três observações, devolvendo valores vazios abaixo desse limiar; três pontos são a condição mínima para resolver o sistema com um grau de liberdade residual, na aceção da equação~\ref{eq:mat-rss-escala}. O mínimo de trinta observações está declarado na dimensão dos vetores, mas não é lido por essa partição: é usado apenas pela medida que compõe o cartão de qualidade das referências, onde decide quais os vetores que aí aparecem --- e a linha do agregado global é apresentada nesse cartão sem sujeição ao mesmo mínimo. A consequência é verificável na tabela publicada: das $583$ células, uma não tem reta por dispor de duas observações e $23$ têm reta estimada com menos de trinta, em quatro unidades. Nenhuma pertence à unidade avaliada nos Capítulos~\ref{cha:diagnostico} e~\ref{cha:discussao}, cujas $48$ células têm entre $119$ e $364$ observações; o resultado dessa avaliação não é, por isso, afetado. A regra de admissibilidade por etapa permanece por unificar num único ponto do modelo, e essa unificação é trabalho de plataforma, não de análise.}

\subsection{Modelos constantes, pela origem e com covariáveis}
\label{sec:mat-modelos-comparadores}
% MAP: M05.04 M05.05
O preditor constante usa $\widehat E_t=\bar
E_{\mathcal R}$\matref{~---~o comparador trivial recomendado para séries
temporais~\parencite{HyndmanKoehler2006}~---}. Uma reta forçada a passar pela origem tem
\begin{equation}
...
\end{equation}
O desenho consultado usa seis blocos de 2020 para os normalizadores
declarados\matref{~\parencite{BergmeirHyndmanKoo2018}}. Como o ajuste dos restantes blocos pode incluir datas posteriores ao bloco omitido, este procedimento estima uma escala por exclusão de blocos; não reproduz uma previsão cronológica dentro de 2020. A avaliação anual para o ano seguinte tem esse segundo papel. Devem declarar-se os dias sem previsão, as condições de suporte e o número de erros reunidos.
...
\matref{Os ganhos estimados, com intervalo por vetor e par anual e o zero
como referência, estão na Figura~\ref{fig:ganho-preditivo} do
Capítulo~\ref{cha:diagnostico}, e a população que os sustenta na
Figura~\ref{fig:amostra-filtros} do mesmo capítulo.}
...
\matref{A Figura~\ref{fig:mat-blocos} representa esta construção num exemplo
com uma lacuna, e o efeito do comprimento do bloco sobre a largura do
intervalo.}

\begin{figure}[tbp]
  \centering
  \includegraphics[width=\textwidth]{gerado/apE-blocos-moveis}
  \caption[\emph{Bootstrap} de blocos móveis no calendário civil]%
  {\matref{O \emph{bootstrap} de blocos móveis no calendário civil. O exemplo
  é construído: nenhum valor da refinaria entra nesta figura. \textbf{A}: um
  calendário de 22 dias, quatro deles sem registo, com $\ell=5$ e $n=18$
  observações. Um bloco candidato é o conjunto das observações cuja data cai
  na janela civil, conforme a equação~\ref{eq:mat-blocos}: $\mathcal B_2$ e
  $\mathcal B_{14}$ trazem cinco observações, $\mathcal B_9$ traz uma.
  \textbf{B}: uma réplica, com o sorteio ilustrativo $\mathcal B_2$,
  $\mathcal B_9$, $\mathcal B_{14}$, $\mathcal B_6$, $\mathcal B_{17}$; as
  barras verticais marcam as junções artificiais entre blocos e as células
  tracejadas são o excesso truncado em $n$. \textbf{C}: sensibilidade ao
  comprimento do bloco, em simulação --- 120 séries AR(1) independentes com
  $\rho=0{,}6$ e $n=240$, 800 réplicas por $\ell$ e semente fixa. Representa-se
  a largura média do intervalo de percentil a 95\,\%, centrada, de modo a
  tornar visível que só a largura responde a $\ell$. Em $\ell=1$ a reamostragem
  trata os dias como independentes e o intervalo sai 41\,\% mais estreito do
  que em $\ell_0=\lceil n^{1/3}\rceil$.}}
  \label{fig:mat-blocos}
\end{figure}

\subsection{Bootstrap sob a hipótese nula}
\label{sec:mat-bootstrap-nulo}
% MAP: M07.07
Para aproximar a distribuição de uma estatística sob $H_0$, ajusta-se primeiro o modelo restrito\matref{~\parencite{Efron1979,EfronTibshirani1993}}. Uma representação do bootstrap residual é
\begin{equation}
...
\matref{O contrato inferencial das tabelas foi confrontado com a implementação e distingue-se como segue. Os intervalos de ganho e de cobertura são intervalos de percentil do bootstrap de blocos móveis: os blocos são definidos no calendário civil, com candidatos em todos os inícios de janela que contenham pelo menos uma observação; as linhas são reamostradas em conjunto, preservando o emparelhamento entre colunas; a estatística é reavaliada em cada réplica com os modelos já treinados mantidos fixos; e a semente deriva de um espaço de nomes declarado, o que torna cada réplica reproduzível. Antes de cada intervalo, o domínio de aplicação é avaliado pela autocorrelação de desfasamento um observada, com três estados declarados: validado abaixo de $0{,}90$ em módulo, sensibilidade exigida entre $0{,}90$ e $0{,}95$, e não suportado a partir de $0{,}95$. A sensibilidade ao comprimento do bloco é calculada em separado, com a estimativa pontual fixa. Os valores $p$ da estabilidade anual têm origem diferente: resultam de testes de Wald sobre contrastes de igualdade de declives e de níveis entre seis anos, com a covariância robusta da equação~\ref{eq:mat-hac}, núcleo quadrático-espectral e largura de banda automática sem multiplicador no chamador, e são corrigidos para a família dos quatro vetores diários --- pelo procedimento de Benjamini--Hochberg na família primária e pelo de Benjamini--Yekutieli na família de verificação. Deve notar-se, por fim, que o bootstrap sob a hipótese nula descrito nesta secção não chegou a ser executado: nenhum intervalo publicado o utiliza, e a palavra ``nulo'', no código de avaliação preditiva, designa o comparador trivial e não uma distribuição de referência.}
...
% MAP: M08.01 M08.09
Na comparação consultada entre dias retidos e removidos, a diferença padronizada entre grupos $A$ e $B$\matref{~\parencite{Cohen1988}} é
\begin{equation}
...
Durbin--Watson\matref{~\parencite{DurbinWatson1951}} e
Ljung--Box\matref{~\parencite{LjungBox1978}} são
\begin{equation}
...
A heterocedasticidade significa que $\operatorname{Var}(e_t\mid\boldsymbol x_t)$ varia com as condições. Na versão studentizada de
Breusch--Pagan/Koenker\matref{~\parencite{BreuschPagan1979,Koenker1981}}, a
regressão auxiliar dos resíduos quadrados nas variáveis declaradas conduz a uma estatística $nR_{\mathrm{aux}}^2$. No diagnóstico ARCH de ordem $h$, a regressão auxiliar inclui $e_{t-1}^2,\ldots,e_{t-h}^2$ e a estatística é $n_{\mathrm{aux}}R_{\mathrm{aux}}^2$. A primeira procura variação associada às covariáveis; a segunda procura persistência na escala. As distribuições assintóticas exigem hipóteses próprias e uma amostra auxiliar admissível.
...
% MAP: M08.07 M08.08
Antes de comparar distâncias, as covariáveis são padronizadas: $Z_{ij}^{\mathrm{std}}=(Z_{ij}-\mu_j)/s_j$, com parâmetros estimados na população de ajuste declarada. A análise de componentes principais\matref{~\parencite{Jolliffe2002}} diagonaliza a matriz de covariância dessa representação. Se $\lambda_k,\boldsymbol v_k$ forem os seus pares próprios, o componente $k$ é $\boldsymbol Z_i^{\mathrm{std}}\boldsymbol v_k$ e a fração de variância explicada é $\lambda_k/\sum_j\lambda_j$.

O algoritmo $k$-means\matref{~\parencite{MacQueen1967}} minimiza a soma das distâncias quadráticas aos centroides:
\begin{equation}
...
% MAP: M08.10 M08.11
A alavancagem é a diagonal $h_{ii}$ da matriz de projeção $H=X(X^{\mathrm T}X)^{-1}X^{\mathrm T}$. A distância de Cook\matref{~\parencite{Cook1977}} combina alavancagem e resíduo:
\begin{equation}
...
\matref{Os graus de liberdade dos testes auxiliares são os declarados na implementação: Ljung--Box compara com $\chi^2_h$ nos desfasamentos de sete e de trinta dias, sem subtrair os parâmetros estimados; a versão studentizada de Breusch--Pagan usa um grau de liberdade; e o diagnóstico ARCH de ordem $h$ usa $h$. Nenhum destes valores $p$ é tratado como calibrado: o código regista-os com o papel explícito de índice descritivo e não os associa a qualquer decisão. Os únicos valores $p$ com calibração sustentada na dissertação são os dos testes de Wald da estabilidade anual, descritos na Secção~\ref{sec:mat-inferencia}, com covariância robusta e correção para a família de vetores. A diferença padronizada entre dias retidos e removidos é igualmente descritiva, e o seu denominador é o declarado na equação~\ref{eq:mat-smd}, com variâncias de divisor $n_g$.}

\matref{O diagnóstico está organizado por pergunta no
Capítulo~\ref{cha:diagnostico}: a relação com a carga e a população
excluída na Figura~\ref{fig:dispersao-energia-carga}, a dependência
temporal dos resíduos na Figura~\ref{fig:dependencia-residuos} e a
influência medida por variação de previsão a carga comum na
Figura~\ref{fig:predicoes-carga-comum}. Os gráficos de componentes
principais e de agrupamento não foram incluídos: sem informação de
processo com que os confrontar, nomear um grupo geométrico como regime
operacional seria uma interpretação que os dados não autorizam.}
...
% MAP: M09.01 M09.02
Uma banda é definida pelo centro $c_t$ e pelos limites $L_t,U_t$. Num modelo linear com erros normais independentes e homocedásticos, o intervalo de confiança da média e o intervalo de predição de uma observação futura em $\boldsymbol x_0$ são, respetivamente\matref{~\parencite{DraperSmith1998}},
\begin{align}
...
\matref{A Figura~\ref{fig:mat-bandas} põe os três objetos no mesmo eixo e
mede o que a banda de largura constante cobre em função de $n$.}

\begin{figure}[tbp]
  \centering
  \includegraphics[width=\textwidth]{gerado/apE-bandas-comparadas}
  \caption[Intervalo da média, intervalo de predição e banda de largura constante]%
  {\matref{Três bandas com o mesmo centro, que respondem a perguntas
  diferentes. O exemplo é construído, em unidades arbitrárias: nenhum valor
  da refinaria entra nesta figura. \textbf{A}: o intervalo de confiança da
  média (equação~\ref{eq:mat-ic-media}) localiza a reta; o intervalo de
  predição (equação~\ref{eq:mat-ip-observacao}) localiza uma observação
  futura; a banda $\pm 2s$ é a convenção da plataforma. O ajuste do exemplo
  usa deliberadamente poucas observações, $n=12$, para que os três objetos
  se distingam: o intervalo da média é aqui $3{,}6$ vezes mais estreito do
  que o de predição no centro do treino, e a banda da plataforma fica
  \emph{dentro} do intervalo de predição em toda a gama. \textbf{B}: a
  semilargura de cada banda em unidades de $s$. Os dois intervalos são
  ampulhetas, porque o termo de alavancagem cresce ao afastar-se de
  $\bar Q$; a banda da plataforma é plana e não responde a essa variação.
  \textbf{C}: a cobertura real da banda $\pm 2s$ para um novo dia, calculada
  exatamente como $2F_{t,n-2}\!\left(2/\sqrt{1+h_0}\right)-1$, com
  $h_0=1/n$ no centro do treino e $h_0=4/n$ no seu extremo sob um desenho
  uniforme; as verticais marcam o mínimo declarado por vetor, 30, e um ano
  civil completo, 365. O défice do painel A é um efeito de amostra pequena:
  com um ano de dados a convenção reproduz o nominal da
  equação~\ref{eq:mat-dois-sigma} com três casas ($95{,}35\,\%$ contra
  $95{,}45\,\%$), ao passo que em $n=30$ cobre $94{,}1\,\%$ no centro e
  $92{,}9\,\%$ no extremo da gama de carga.}}
  \label{fig:mat-bandas}
\end{figure}
...
7 & 15 & Todos os $|z_i|<1$.\\
8 & 8 & \matref{Todos os $|z_i|>1$, em qualquer dos lados.}\\\bottomrule
\end{tabularx}
\end{table}
...
\matref{As convenções foram confirmadas na implementação da plataforma e são as da Tabela~\ref{tab:mat-regras}. A regra de mesmo lado usa oito pontos e não nove, e exige sinal estrito: um resíduo exatamente nulo não conta para nenhum dos lados, pelo que interrompe a sequência em vez de a prolongar. A regra~8 conta os oito pontos pelo módulo do resíduo padronizado, admitindo pontos em lados opostos da referência desde que todos excedam um desvio-padrão; não é, portanto, uma regra de lateralidade, e descrevê-la como tal seria incorreto. Todas as comparações usam desigualdade estrita. A consecutividade é avaliada sobre a ordem dos dias retidos pela seleção e não sobre o calendário: um dia excluído não interrompe a sequência, é ignorado, de modo que oito pontos consecutivos podem cobrir mais de oito dias civis. As tabelas de alarmes contam dias sinalizados e violações de regra; a noção de episódio acima definida não está implementada e é aqui apresentada como convenção a adotar, não como descrição do que a plataforma conta.}
...
\matref{A comparação ilustrativa entre os três tipos de banda está na
Figura~\ref{fig:mat-bandas}; a comparação empírica, com a banda marginal, o
centro autorregressivo e a cobertura medida nos dados da unidade, está na
Figura~\ref{fig:cobertura-bandas} do Capítulo~\ref{cha:diagnostico}.}
...
\matref{A Figura~\ref{fig:mat-conjunto} desenha esse exemplo e enumera todas
as configurações em que as duas regras divergem.}

\begin{figure}[tbp]
  \centering
  \includegraphics[width=\textwidth]{gerado/apE-separado-vs-conjunto}
  \caption[Contagens separadas e sucesso conjunto na regra de quatro em cinco]%
  {\matref{As somas marginais não determinam a contagem conjunta. Os dois
  exemplos são construídos; nenhum valor da refinaria entra nesta figura.
  Em ambos, quatro dos cinco pares anuais encadeados têm ganho que exclui
  zero e quatro têm cobertura compatível, pelo que a regra da
  equação~\ref{eq:mat-gate-historico} se cumpre nos dois casos. \textbf{A}:
  o ganho falha no último par e a cobertura no primeiro; como falham pares
  diferentes, só três pares satisfazem os dois critérios ao mesmo tempo.
  \textbf{B}: os mesmos totais, mas o par em falta é o mesmo nos dois
  vetores, e a contagem conjunta sobe a quatro. Uma regra escrita como
  $\sum_jG_jC_j\geq4$ separaria estes dois casos; a regra histórica não os
  separa. A diferença não é uma curiosidade de um exemplo: das 36
  configurações em que ambos os vetores somam quatro ou cinco, 20 ---
  $56\,\%$ --- falhariam a versão conjunta com o mesmo limiar nominal. Daí
  a exigência de identificar qual das duas definições foi usada ao
  reproduzir um resultado histórico.}}
  \label{fig:mat-conjunto}
\end{figure}

\matref{Os dois critérios não incidem sobre o mesmo objeto, e é esse o alcance do resultado histórico. O critério de ganho compara a previsão anual do ajuste oficial com o comparador trivial treinado no mesmo período: o seu centro é $\widehat f(\boldsymbol x_t)$. O critério de cobertura é avaliado sobre a banda do candidato e, na rota autorregressiva, essa banda tem por centro $c_t^{AR}$ da equação~\ref{eq:mat-centro-ar}, que acrescenta ao ajuste a previsão condicional do resíduo e regressa ao centro marginal nos dias sem os desfasamentos civis necessários. Uma aceitação certifica, assim, duas proposições distintas --- que o ajuste anual tem conteúdo preditivo sobre o comparador trivial, e que existe uma banda cuja cobertura é compatível com o valor nominal --- e não que um único preditor satisfaz ambas. Para uma finalidade preditiva, uma nova avaliação deve calcular a perda do preditor completo, incluindo os dias de substituição marginal, e aplicar os dois critérios a esse mesmo objeto. Os resultados já publicados conservam-se com a definição com que foram produzidos, e é com essa definição que devem ser lidos.}
...
\matref{A diferença entre contagens separadas e sucesso conjunto está na
Figura~\ref{fig:mat-conjunto}; a matriz de decisão por candidato e par
anual, com os estados efetivamente observados, está na
Figura~\ref{fig:matriz-decisao} do Capítulo~\ref{cha:discussao}. As margens
materiais das equações acima permanecem por definir e são critério de uma
avaliação futura, não deste resultado.}
...
\matref{O comportamento de cada canal perante um degrau conhecido está na
Figura~\ref{fig:injeccao} do Capítulo~\ref{cha:discussao}, gerada por
simulação com a amplitude e o instante de mudança declarados. A perda de
sinal por sobreposição da janela de reajuste com o degrau é a
equação~\ref{eq:mat-projecao-sinal}, cujo efeito o mesmo capítulo
quantifica.}
...
% MAP: M12.03
Para início $\tau$, amplitude $a\sigma_{\mathrm{DGP}}$ e duração de
crescimento $d$, duas alternativas\matref{~---~as duas famílias canónicas da
literatura de deteção de mudança~\parencite{Lai1995}~---} são
\begin{align}
...
% MAP: M12.05
\matref{O tempo até ao primeiro sinal é a grandeza que as cartas de controlo
resumem no comprimento médio de série~\parencite{Page1954,Lai1995}.} Para uma
mudança que começa em $\tau$, o primeiro cruzamento posterior é $T_A=\inf\{t\geq\tau:T_t>h\}$. O atraso civil é $D=T_A-\tau$; uma deteção no próprio dia tem atraso zero. Se o estudo contar oportunidades pontuáveis, deve usar outro índice e identificar a unidade. Para uma janela que termina em $\tau+H-1$, a probabilidade de deteção é
\begin{equation}
...
\label{sec:mat-mdd}
% MAP: M12.06
Para probabilidade-alvo $\pi_0$, define-se o desvio mínimo detetável\matref{~---~o
análogo, em unidades de dispersão, do tamanho de efeito mínimo de uma análise
de potência~\parencite{Cohen1988}~---} por
\begin{equation}
...
% MAP: M12.07
Dois instrumentos devem receber a mesma realização de covariáveis, ruído,
máscara e mudança\matref{~\parencite{EfronTibshirani1993}}. Se $I_{A,r},I_{B,r}$ assinalarem deteção, a diferença por realização é $d_r=I_{A,r}-I_{B,r}$ e
\begin{equation}
...
% MAP: M12.08
\matref{A extensão multivariada do processo autorregressivo segue a
formulação vetorial clássica~\parencite{BoxJenkins2015}.} Para $V$ vetores,
seja $\Sigma=D_\sigma C D_\sigma$ a covariância marginal, com matriz de correlação $C$ e $D_\sigma=\operatorname{diag}(\sigma_1,\ldots,\sigma_V)$. Com persistência comum $\rho$, uma construção estacionária é
\begin{equation}
...
\matref{Três coisas devem manter-se separadas: a formulação de um cenário, a sua execução sobre um lote válido e um resultado consolidado. Este apêndice descreve os instrumentos; a presença de uma formulação aqui não converte um cenário previsto em resultado executado, nem um lote provisório em evidência consolidada. Cada cenário citado no capítulo de diagnóstico remete para o lote e o manifesto que o produziram, e as condições que o definem --- treino, horizonte, máscara, número de realizações, separação entre calibração e verificação, convenção de censura e família de contrastes --- acompanham esse registo e não este texto.}

\clearpage
\matref{As curvas de probabilidade de deteção por amplitude, com a
incerteza Monte Carlo e a censura do menor desvio detetável visível, estão
na Figura~\ref{fig:deteccao} do Capítulo~\ref{cha:discussao}. São curvas de
simulação, e é essa a sua leitura: a detetabilidade medida num gerador
declarado não é uma taxa observada nos alarmes da refinaria.}
