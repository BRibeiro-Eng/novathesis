...
de estimandos, candidatos e simulação para o Cap. 4/apêndice de matemática; manter
...
O que o critério aceita tem de ficar claro antes dos números. Aceita um par
formado por um preditor da relação com a carga, sob a hipótese de que ela se
transporta para o ano seguinte, e uma banda calibrada sob a hipótese de que
nada mudou. A referência congelada da Secção~\ref{sec:referencia-congelada}
lê a mesma perda de cobertura no ano seguinte como um sinal a preservar, e
sem eventos rotulados os dados não separam as duas leituras. A aceitação de
um candidato não o certificaria, por isso, como detetor, e a rejeição de
todos não retira utilidade a uma referência usada para medir desvios; é
esta a razão das duas pistas da Secção~\ref{sec:implicacoes-plataforma}.
...
marginal quando o histórico não chega (124 dos $6\,580$ dias avaliados nas
duas construções, $1{,}9\%$); produziu duas construções, uma
...
banda, são dezasseis decisões. O par junta dois objetos julgados em
separado: o normalizador, a reta anual, pelo conteúdo preditivo do
